...
if data.seq.frame > 1
    ...
    while iter <= params.refinement_iterations && any(old_pos ~= data.obj.pos)
        ...
        if cle<1 && max_response>max_tmp
           max_tmp = max_response;
        end
           
        if max_response< thr_1%&& params.selector~=-1 %% cle>5
...
%                     trainstate = 1;
        ...
        elseif max_response> thr_2 && cle<=2 %0.5*max_tmp
            if ~slt_frames(data.seq.frame)
                num_sample = numel(train_set);
                train_set(num_sample+1).response = ifftshift(slt_response);
...
%                     trainstate = 1;
        ...
        elseif max_response>=thr_1 && max_response<=thr_2 %&& cle<1%&& params.selector~=1
            if ~slt_frames(data.seq.frame)
                num_sample = numel(train_set);
                train_set(num_sample+1).response = ifftshift(slt_response);
                train_set(num_sample+1).gt_sltor = 1;
